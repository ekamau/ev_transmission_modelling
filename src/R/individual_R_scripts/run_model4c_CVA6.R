...
# initialize values:
initf <- function(chain_id = 1) { list(lambda = list(runif(n = (CA6_dat$N)+yr3-yr1 , min=0.01, max=0.5)),
...
fit <- stan(
  file = "src/stan/model4.stan", data = CA6_dat, chains = 4, 
...
saveRDS(fit.summary, file = "results/fitSummary_CVA6_model4c.rds")
write.csv(fit.summary, file = "results/fitSummary_CVA6_model4c.csv")
