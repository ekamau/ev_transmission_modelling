...
CA6_dat <- list(N = 80, yr3 = yr3, yr2 = yr2, yr1 = yr1, 
...
# initialize values:
initfn <- function(chain_id = 1) { list(lambda = list(runif(n = (yr3 - CA6_dat$max_yr_cst), min = 0.01, max = 0.5)),
                                       sigma = list(rexp(1, 1)), rho = list(rexp(1, 1)), lambda_cst = list(rexp(1, 1))) }

n_chains <- 4
init_ll <- lapply(1:n_chains, function(id) initfn(chain_id = id))

# OR:
# initfn <- function() {
#   list(lambda = list(runif(n = (CA6_dat$N) + yr3 - yr1 , min = 0.01, max = 0.5)),
#        sigma = list(rexp(1, 0.1)), rho = list(rexp(1, 0.1)))
# }
# 
# opt <- rstan::optimizing(model_v4, data = CA6_dat, init = initfn, as_vector = FALSE)
# initf <- function() { list( lambda = opt$par$lambda, sigma = opt$par$sigma) }

##-- Model fitting --##
# method 1 - MPS
fit <- stan(file = "src/stan/model3_4_31Oct23/model4_corrected_3_extension_opt-estimateRho-EK.stan", data = CA6_dat, chains = 4, init = init_ll,  warmup = 3000, 
                  iter = 10000, refresh = 5000, 
                  control = list(adapt_delta = 0.9999, max_treedepth = 25))

fit <- stan(file = "src/stan/model3_4_31Oct23/model4_corrected_3_extension_opt-estimateRho-EK.stan", data = CA6_dat, chains = 4, init = "random",  warmup = 3000, 
            iter = 10000, refresh = 5000, 
            control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/model3_4_31Oct23/fit_CA6_model4V4_estimateRho-31Oct23.rds")
fit <- readRDS(file = "results/archived_Apr24/model3_4_31Oct23/fit_CA6_model4V4_estimateRho-31Oct23.rds")

# method 2 - BL
# fit <- rstan::sampling(model_v4, data = CA6_dat, chains = 4, init = initf, warmup = 3000, 
#                        iter = 10000, refresh = 5000, 
#                        control = list(adapt_delta = 0.9999, max_treedepth = 25))
# 
...
# waic:
waic(logLik_fit)
waic_m4 <- loo::waic(logLik_fit)
sink('results/CVA6/model4/waic_CA6_m4.txt'); print(waic_m4); sink()
...
sink('results/CVA6/model4/loo_CA6_m4.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/CVA6/model4/fitSummary_CA6_model4.rds")
write.csv(fit.summary, file = "results/CVA6/model4/fitSummary_CA6_model4.csv")
