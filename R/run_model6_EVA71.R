...
E71_dat <- list(N = 80, n_2006 = df_2006$n, z_2006 = df_2006$seropositive, 
                n_2011 = df_2011$n, z_2011 = df_2011$seropositive, 
                n_2017 = df_2017$n, z_2017 = df_2017$seropositive)

# initialize values:
initf <- function(chain_id = 1) { list(lambda = list(rexp(1, 1)), beta = list(rexp(1, 20))) }
n_chains <- 4
...
fit <- stan(
  file = "src/stan/model6_exponential10.stan", data = E71_dat, chains = 4, 
  init = "random",  warmup = 3000, iter = 10000, refresh = 5000, 
  control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/EVA71/model6/E71_model6_fit-exp10.rds")

# extract parameters from fit:
fit_lambda_rho_beta <- extract(fit, c("lambda", "beta", "rho"))
saveRDS(fit_lambda_rho_beta, file = "results/EVA71_model6_lambda_rho_beta.rds")
...
sink('results/EVA71/model6/waic_E71_m6-exp10.txt'); print(waic_m6); sink()
...
sink('results/EVA71/model6/loo_E71_m6-exp10.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/fitSummary_EVA71_model6_14Jul23.rds")
write.csv(fit.summary, file = "results/fitSummary_EVA71_model6_14Jul23.csv")
