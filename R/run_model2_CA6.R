...
fit <- stan(
  file = "src/stan/model2_exponential10.stan", data = CA6_dat, chains = 4, 
  init = "random",  warmup = 3000, iter = 10000, refresh = 5000, 
  control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/CVA6/model2/CA6_model2_fit-exp10.rds")

# extract parameters from fit:
fit_lambda_rho <- extract(fit, c("lambda", "rho"))
saveRDS(fit_lambda_rho, file = "results/CVA6/model2/CA6_model2_lambda_rho-exp10.rds")
...
sink('results/CVA6/model2/waic_CA6_m2-exp10.txt'); print(waic_m2); sink()
...
sink('results/CVA6/model2/loo_CA6_m2-exp10.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/fitSummary_CVA6_model2.rds")
write.csv(fit.summary, file = "results/fitSummary_CVA6_model2.csv")
