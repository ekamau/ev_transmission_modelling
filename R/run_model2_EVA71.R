...
fit <- stan(
  file = "src/stan/model2_exponential10.stan", data = E71_dat, chains = 4, 
  init = "random",  warmup = 3000, iter = 10000, refresh = 5000, 
  control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/EVA71/model2/E71_model2_fit-exp10.rds")

# extract parameters from fit:
fit_lambda_rho <- extract(fit, c("lambda", "rho"))
saveRDS(fit_lambda_rho, file = "results/EVA71_model2_lambda_rho.rds")
...
# waic:
waic(logLik_fit)
waic_m2 <- loo::waic(logLik_fit)
sink('results/EVA71/model2/waic_E71_m2-exp10.txt'); print(waic_m2); sink()
...
sink('results/EVA71/model2/loo_E71_m2-exp10.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/fitSummary_EVA71_model2.rds")
write.csv(fit.summary, file = "results/fitSummary_EVA71_model2.csv")
