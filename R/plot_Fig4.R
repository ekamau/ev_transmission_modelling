...
plot_fig4 <- function(){
  ...
  c <- ggplot() +
    ...
    geom_line(data = df_fitCA6, aes(x = age, y = estProp), color = "#D95F02", linewidth=0.8) +
  ...
  fit_CA6B <- read.csv('data/fitSummary_CA6_M4.csv', header = TRUE, sep = ',')
  ...
  d <- ggplot() +
    geom_errorbar(data = df_fitCA6B, aes(x = age, ymin = estProplow, ymax = estPropup), 
                  color = "#D95F02", width = 0.7, size = 0.5) +
    geom_line(data = df_fitCA6B, aes(x = age, y = estProp), color = "#D95F02", linewidth=0.8) +
  ...
  prevAgeE71 <- dat_EVA71 %>% group_by(Year_collection, age_rounded) %>% 
    filter(seroStatus == "Positive" & age_rounded <= 80) %>% reframe(seropositive = n())
  ...
  a <- ggplot() +
    ...
    geom_line(data = df_fitE71, aes(x = age, y = estProp), color = "#1B9E77", linewidth=0.8) +
  ...
  fit_E71B <- read.csv('data/fitSummary_EVA71_M4.csv', header = TRUE, sep = ',')
  ...
  b <- ggplot() +
    geom_errorbar(data = df_fitE71B, aes(x = age, ymin = estProplow, ymax = estPropup), 
                  color = "#1B9E77", width = 0.7, size = 0.5) +
    geom_line(data = df_fitE71B, aes(x = age, y = estProp), color = "#1B9E77", linewidth=0.8) +
  ...
  fig4 <- a / b / c / d
  ggsave(filename = 'results/figures/Figure4.pdf', fig4, height = 200, width = 183, units = 'mm', dpi = 300)
