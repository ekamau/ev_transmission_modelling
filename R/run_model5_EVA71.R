...
fit <- stan(
  file = "src/stan/model5_exponential10.stan", data = E71_dat, chains = 4, 
  init = "random",  warmup = 3000, iter = 10000, refresh = 5000, 
  control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/EVA71/model5/E71_model5_fit-exp10.rds")

# extract parameters from fit:
fit_lambda_beta <- extract(fit, c("lambda", "beta"))
saveRDS(fit_lambda_beta, file = "results/EVA71/model5/E71_m5_lambda_beta.rds")
...
# waic:
waic(logLik_fit)
waic_m5 <- loo::waic(logLik_fit)
sink('results/EVA71/model5/waic_E71_m5-exp10.txt'); print(waic_m5); sink()
...
sink('results/EVA71/model5/loo_E71_m5-exp10.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/fitSummary_EVA71_model5.rds")
write.csv(fit.summary, file = "results/fitSummary_EVA71_model5.csv")
