...
# initialize values:
initf <- function(chain_id = 1) { list(lambda = list(rexp(1, 1))) }
n_chains <- 4
...
fit <- stan(
  file = "src/stan/model1_exponential10.stan", data = CA6_dat, chains = 4, 
  init = init_ll,  warmup = 3000, iter = 10000, refresh = 5000, 
  control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/CVA6/model1/CA6_model1_fit-exp10.rds")

# extract parameters from fit:
fit_lambda <- extract(fit, c("lambda"))
saveRDS(fit_lambda, file = "results/CVA6/model1/CA6_model1_lambda-exp10.rds")
...
# waic:
#waic1 <- waic(logLik_fit)
waic_m1 <- loo::waic(logLik_fit)
sink('results/CVA6/model1/waic_CA6_m1-exp10.txt'); print(waic_m1); sink()
...
sink('results/CVA6/model1/loo_CA6_m1-exp10.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/fitSummary_CVA6_model1.rds")
write.csv(fit.summary, file = "results/fitSummary_CVA6_model1.csv")
