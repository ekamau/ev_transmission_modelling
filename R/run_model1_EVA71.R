...
fit <- stan(
  file = "src/stan/model1_exponential10.stan", data = E71_dat, chains = 4, 
  init = "random",  warmup = 3000, iter = 10000, refresh = 5000, 
  control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/EVA71/E71_model1-exp10_fit.rds")

# extract parameters from fit:
fit_lambda <- extract(fit, c("lambda"))
saveRDS(fit_lambda, file = "results/EVA71/model1/E71_model1-exp10_lambda.rds")
...
sink('results/EVA71/model1/waic_E71_m1-exp10.txt'); print(waic_m1); sink()
...
sink('results/EVA71/model1/loo_E71_m1-exp10.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/fitSummary_EVA71_model1.rds")
write.csv(fit.summary, file = "results/fitSummary_EVA71_model1.csv")
