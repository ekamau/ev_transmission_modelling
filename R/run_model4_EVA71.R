...
# OR:
# initfn <- function() {
#   list(lambda = list(runif(n = (E71_dat$N)+yr3-yr1 , min = 0.01, max = 0.5)),
#        sigma = list(rexp(1, 0.1))) 
# }
# 
# model_v4 <- stan_model("")
# opt <- rstan::optimizing(model_v4, data = E71_dat, as_vector = FALSE)
# initf <- function() { list( lambda = opt$par$lambda, sigma = opt$par$sigma) }


##-- Model fitting --##
# method 1 - MPS
fit <- stan(file = "src/stan/model3_4_31Oct23/model4_corrected_3_extension_opt-estimateRho-EK.stan", data = E71_dat, chains = 4, init = init_ll,  warmup = 3000, 
            iter = 10000, refresh = 5000, 
            control = list(adapt_delta = 0.9999, max_treedepth = 25))

saveRDS(fit, file = "results/model3_4_31Oct23/fit_EVA71_model4V4-estimateRho-31Oct23.rds")
fit <- readRDS(file = "results/archived_Apr24/model3_4_31Oct23/fit_EVA71_model4V4-estimateRho-31Oct23.rds")
# method 2 - BL
# fit <- rstan::sampling(model_v4, data = E71_dat, chains = 4, init = initf, warmup = 3000, 
#                        iter = 10000, refresh = 5000, 
#                        control = list(adapt_delta = 0.9999, max_treedepth = 25))
# 
...
sink('results/EVA71/model4/waic_E71_m4.txt'); print(waic_m4); sink()
...
sink('results/EVA71/model4/loo_E71_m4.txt'); print(loo_fit); sink()
...
saveRDS(fit.summary, file = "results/EVA71/model4/fitSummary_E71_m4.rds")
write.csv(fit.summary, file = "results/EVA71/model4/fitSummary_E71_m4.csv")
